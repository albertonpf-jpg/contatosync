...
async function discoverICloudAddressBook(appleId, appPassword) {
    ...
    try {
        ...
        debugLog('Principal PROPFIND status: ' + principalResp.status);
        debugLog('Principal PROPFIND data: ' + String(principalResp.data).substring(0, 500));

        if (principalResp.status === 401) throw new Error('Credenciais inválidas');

        const principalMatch = String(principalResp.data).match(/<[^>]*:?href[^>]*>\s*([^<]+)\s*<\/[^>]*:?href>/i);
        if (!principalMatch) throw new Error('sem principal href');

        let principalHref = principalMatch[1].trim();
        debugLog('Principal href extraído: ' + principalHref);

        const homeUrl = principalHref.startsWith('http') ? principalHref : 'https://contacts.icloud.com' + principalHref;
        debugLog('Consultando home-set em: ' + homeUrl);
        ...
        if (homeMatch) {
            ...
            try {
                const listResp = await axios({
                    ...
                    auth: { username: appleId, password: appPassword },
                    headers: { 'Content-Type': 'application/xml; charset=utf-8', 'Depth': '1' },
                    data: '<?xml version="1.0" encoding="utf-8"?><D:propfind xmlns:D="DAV:" xmlns:C="urn:ietf:params:xml:ns:carddav"><D:prop><D:resourcetype/><D:displayname/></D:prop></D:propfind>',
                    timeout: 15000,
                    validateStatus: (s) => s < 600
                });

                debugLog('List addressbooks status: ' + listResp.status);
                debugLog('List addressbooks data: ' + String(listResp.data).substring(0, 1000));

                const abMatches = [...String(listResp.data).matchAll(/<[^>]*:?href[^>]*>\s*([^<]+)\s*<\/[^>]*:?href>/gi)];
                for (const m of abMatches) {
                    const href = m[1].trim();
                    if (href !== homeHref && href.length > homeHref.length - 5) {
                        let abUrl = href.startsWith('http') ? href : 'https://contacts.icloud.com' + href;
                        if (!abUrl.endsWith('/')) abUrl += '/';
                        debugLog('Usando addressbook URL: ' + abUrl);
                        return abUrl;
                    }
                }
            } catch (e) {
                debugLog('Erro ao listar addressbooks: ' + e.message);
            }

            return homeHref;
        }
    } catch (e) {
        debugLog('Erro na descoberta principal: ' + e.message);
    }

    // Fallback: extrair ID numérico
    try {
        const redirectResp = await axios({
            method: 'PROPFIND',
            url: 'https://contacts.icloud.com/',
            auth: { username: appleId, password: appPassword },
            headers: { 'Content-Type': 'application/xml; charset=utf-8', 'Depth': '1' },
            data: '<?xml version="1.0" encoding="utf-8"?><D:propfind xmlns:D="DAV:"><D:prop><D:href/></D:prop></D:propfind>',
            timeout: 15000,
            validateStatus: (s) => s < 600,
            maxRedirects: 5
        });
        const numericMatch = String(redirectResp.data).match(/\/(\d{10,})\//);
        if (numericMatch) {
            const fallbackUrl = 'https://contacts.icloud.com/' + numericMatch[1] + '/carddavhome/card/';
            debugLog('Fallback URL numérico: ' + fallbackUrl);
            return fallbackUrl;
        }
    } catch (e) {
        debugLog('Erro no fallback: ' + e.message);
    }

    const lastFallback = 'https://contacts.icloud.com/' + appleId.replace('@icloud.com', '').replace('@me.com', '') + '/carddavhome/card/';
    debugLog('Último fallback URL: ' + lastFallback);
    return lastFallback;
}
