...
async function initWhatsApp() {
    try {
        ...
        // Handler para mensagens (detectar novos contatos)
...
async function syncGoogleContacts() {
    if (!appState.google.connected) {
        throw new Error('Google não está conectado');
    }
    ...
    try {
        ...
        contacts.forEach(contact => {
            ...
            if (phone) {
                ...
                if (!existingContact) {
                    addNewContact(cleanPhone, name, 'google');
                }
            }
        });
...
async function connectICloud(appleId, appPassword) {
    try {
        ...
        if (error.response && error.response.status === 401) {
            throw new Error('Credenciais incorretas. Verifique Apple ID e senha de app');
        }
...
async function synciCloudContacts(appleId, appPassword) {
    try {
        ...
        if (response.status === 207) {
            ...
            // Processar contatos vCard
            if (result.multistatus && result.multistatus.response) {
                for (const responseItem of result.multistatus.response) {
                    if (responseItem.propstat && responseItem.propstat[0].prop && 
                        responseItem.propstat[0].prop[0]['address-data']) {
                        ...
                        if (contact.phone) {
                            const existingContact = appState.contacts.find(c => 
                                c.phone.includes(contact.phone.replace(/\D/g, ''))
                            );
                            
                            if (!existingContact) {
                                addNewContact(contact.phone, contact.name, 'icloud');
                                contactCount++;
                            }
                        }
                    }
...
function parseVCard(vcard) {
    ...
    lines.forEach(line => {
        ...
        if (line.startsWith('TEL:') || line.includes('TYPE=CELL') || line.includes('TYPE=MOBILE')) {
...
app.post('/whatsapp/connect', async (req, res) => {
    try {
        ...
        res.json({
            ok: true,
            message: 'Conectando WhatsApp... QR Code será gerado em breve',
            instructions: 'Aguarde o QR Code aparecer e escaneie com seu WhatsApp'
        });
...
app.post('/whatsapp/disconnect', (req, res) => {
    try {
        if (appState.whatsapp.socket) {
            appState.whatsapp.socket.logout();
        }
        ...
        res.json({ ok: true }); // Sempre retorna sucesso para reset
    }
});
        ...
        res.status(500).send(`
            <html>
                <body style="font-family:Arial;text-align:center;padding:50px;">
                    ...
                    <p>Google OAuth não está configurado. Configure as variáveis GOOGLE_CLIENT_ID e GOOGLE_CLIENT_SECRET.</p>
...
app.get('/auth/google/callback', async (req, res) => {
    try {
        ...
        res.send(`
            <html>
                <head><title>Google OAuth - ContatoSync</title></head>
                <body style="font-family:Arial;text-align:center;padding:50px;background:#0a0a0a;color:white;">
                    ...
                    <p style="color:#25d366;">Você pode fechar esta janela.</p>
                    <script>
                        setTimeout(() => {
                            window.close();
                        }, 3000);
...
app.post('/google/sync', async (req, res) => {
    try {
        if (!appState.google.connected) {
            return res.json({
                ok: false,
                error: 'Google não está conectado'
            });
        }
...
app.post('/icloud/sync', async (req, res) => {
    try {
        if (!appState.icloud.connected) {
            return res.json({
                ok: false,
                error: 'iCloud não está conectado'
            });
        }
                ...
                <html>
                    <head>
                        ...
                        <style>
                            ...
                            h1 { color: #25d366; font-size: 2.5rem; margin-bottom: 20px; }
                            .status { background: #1a1a1a; border: 1px solid #333; border-radius: 12px; padding: 20px; margin: 20px 0; }
                            .btn { background: #25d366; color: white; border: none; padding: 12px 24px; border-radius: 8px; text-decoration: none; display: inline-block; margin: 10px; }
                            .real { color: #25d366; font-weight: bold; }
                        </style>
                    </head>
                    <body>
                        <div class="container">
                            ...
                            <div class="status">
                                ...
                                <p>Servidor funcionando em <span class="real">MODO PRODUÇÃO</span>!</p>
                                <p><strong>Integrações:</strong></p>
                                <ul style="text-align:left;">
                                    <li>WhatsApp: Baileys (API Real)</li>
                                    <li>Google: OAuth 2.0 (API Real)</li>
                                    <li>iCloud: CardDAV (API Real)</li>
                                </ul>
                            </div>
